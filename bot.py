...
async def cmd_list(message: Message):
    ...
    for s in stations:
        if s["region"] != current_region:
            ...
            regions_seen.append(current_region)
            lines.append(f"\n📍 *{current_region}*")
        latest = get_latest_report(s["id"])
    ...
    kb_rows = [
        [InlineKeyboardButton(text=f"🔄 Обновить: {region}", callback_data=f"region_menu:{region}")]
        for region in regions_seen
    ]
    kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
    await message.answer(
        "\n".join(lines).strip() + "\n\nВыбери район, чтобы быстро обновить статус станции:",
        reply_markup=kb,
        parse_mode="Markdown",
...
async def region_menu(callback: CallbackQuery):
    ...
    for s in stations:
        ...
        kb_rows.append([InlineKeyboardButton(
            text=f"{icon} {s['name']}, {s['address']}", callback_data=f"quick_report:{s['id']}"
        )])
    kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
    await callback.message.answer(f"📍 {region} — жми на станцию:", reply_markup=kb)
